    ...
    <div className="w-56 bg-editor-surface border-l border-editor-border flex flex-col overflow-y-auto flex-shrink-0">
      ...
      {!selectedEl ? (
        ...
        <div className="p-3 space-y-2">
          ...
          <PropRow label="Rotation">
            <div className="flex items-center gap-1">
              <input type="range" min="-180" max="180" value={selectedEl.rotation}
                onChange={(e) => update({ rotation: parseFloat(e.target.value) })} className="flex-1" />
              <span className="text-[10px] w-8 text-right">{selectedEl.rotation}&deg;</span>
            </div>
          </PropRow>
          <PropRow label="Opacity">
            <div className="flex items-center gap-1">
              <input type="range" min="0" max="1" step="0.05" value={selectedEl.opacity}
                onChange={(e) => update({ opacity: parseFloat(e.target.value) })} className="flex-1" />
          ...
          {selectedEl.type === 'documentBody' && <DocumentBodyProperties el={selectedEl as any} update={update} />}
          ...
          <div className="grid grid-cols-2 gap-1">
            ...
            }} className="px-2 py-1.5 text-[10px] bg-red-500/15 text-red-400 rounded hover:bg-red-500/30 transition-colors flex items-center justify-center gap-1">
            ...
            }} className="px-2 py-1.5 text-[10px] bg-editor-bg text-editor-text rounded hover:bg-editor-hover transition-colors flex items-center justify-center gap-1">
              <Copy size={11} /> Duplicate
            </button>
            <button onClick={() => update({ locked: !selectedEl.locked })}
              className="px-2 py-1.5 text-[10px] bg-editor-bg text-editor-text rounded hover:bg-editor-hover transition-colors flex items-center justify-center gap-1">
              {selectedEl.locked ? <><Unlock size={11} /> Unlock</> : <><Lock size={11} /> Lock</>}
            </button>
            <button onClick={() => update({ visible: !selectedEl.visible })}
              className="px-2 py-1.5 text-[10px] bg-editor-bg text-editor-text rounded hover:bg-editor-hover transition-colors flex items-center justify-center gap-1">
              {selectedEl.visible ? <><EyeOff size={11} /> Hide</> : <><Eye size={11} /> Show</>}
            </button>
          </div>
        </div>
      )}
...
function TextProperties({ el, update }: { el: any; update: (u: any) => void }) {
  return (
    <>
      ...
      <textarea value={el.content} onChange={(e) => update({ content: e.target.value })}
      ...
      <PropRow label="Size"><NumberInput value={el.fontSize} onChange={(v) => update({ fontSize: v })} /></PropRow>
      <PropRow label="Color"><ColorInput value={el.color} onChange={(v) => update({ color: v })} /></PropRow>
      <PropRow label="Align">
        <ButtonGroup options={[{ value: 'left', label: 'L' }, { value: 'center', label: 'C' }, { value: 'right', label: 'R' }, { value: 'justify', label: 'J' }]}
          value={el.align} onChange={(v) => update({ align: v })} />
      </PropRow>
      <PropRow label="Style">
      ...
      <PropRow label="Line H."><NumberInput value={el.lineHeight} onChange={(v) => update({ lineHeight: v })} step={0.1} /></PropRow>
    </>
  );
}

function DocumentBodyProperties({ el, update }: { el: any; update: (u: any) => void }) {
  return (
    <>
      <SectionTitle>Document Body</SectionTitle>
      <PropRow label="Font">
        <select value={el.font} onChange={(e) => update({ font: e.target.value })}
          className="w-full bg-editor-bg text-editor-text text-xs border border-editor-border rounded px-1.5 py-1 focus:border-editor-accent outline-none"
          style={{ fontFamily: el.font }}>
      ...
      <div className="grid grid-cols-2 gap-1">
        <PropRow label="Left"><NumberInput value={el.marginLeft ?? 72} onChange={(v) => update({ marginLeft: v, x: v, width: (useDocumentStore.getState().pages[useDocumentStore.getState().activePage]?.width || 595) - v - (el.marginRight ?? 72) })} /></PropRow>
        <PropRow label="Right"><NumberInput value={el.marginRight ?? 72} onChange={(v) => update({ marginRight: v, width: (useDocumentStore.getState().pages[useDocumentStore.getState().activePage]?.width || 595) - (el.marginLeft ?? 72) - v })} /></PropRow>
        <PropRow label="Top"><NumberInput value={el.marginTop ?? 72} onChange={(v) => update({ marginTop: v, y: v })} /></PropRow>
      </div>
    </>
  );
}
...
function ShapeProperties({ el, update }: { el: any; update: (u: any) => void }) {
  return (
    <>
      ...
      <PropRow label="Fill"><ColorInput value={el.fill?.color || '#FFFFFF'} onChange={(v) => update({ fill: { type: 'solid', color: v } })} /></PropRow>
      <PropRow label="Stroke"><ColorInput value={el.stroke?.color || '#000000'} onChange={(v) => update({ stroke: { ...el.stroke, color: v } })} /></PropRow>
      <PropRow label="Stroke W"><NumberInput value={el.stroke?.width || 1} onChange={(v) => update({ stroke: { ...el.stroke, width: v } })} step={0.5} /></PropRow>
      <PropRow label="Radius"><NumberInput value={el.borderRadius || 0} onChange={(v) => update({ borderRadius: v })} /></PropRow>
      <PropRow label="Dash">
...
function ImageProperties({ el, update }: { el: any; update: (u: any) => void }) {
  ...
  const loadFromUrl = () => {
    ...
    img.onload = () => { update({ src: imageUrl.trim() }); setShowUrlInput(false); setImageUrl(''); setUrlLoading(false); };
    img.onerror = () => { update({ src: imageUrl.trim() }); setShowUrlInput(false); setImageUrl(''); setUrlLoading(false); };
  ...
  return (
    <>
      ...
      <button onClick={() => {
        ...
        input.onchange = (e) => { const file = (e.target as HTMLInputElement).files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => update({ src: reader.result as string }); reader.readAsDataURL(file); };
        input.click();
      }} className="w-full px-2 py-1.5 text-[10px] bg-editor-bg text-editor-text rounded hover:bg-editor-hover transition-colors flex items-center gap-1">
        <RefreshCw size={11} /> Replace Image (File)
      </button>
      <button onClick={() => setShowUrlInput(!showUrlInput)}
        className="w-full px-2 py-1.5 text-[10px] bg-editor-bg text-editor-text rounded hover:bg-editor-hover transition-colors mt-1 flex items-center gap-1">
        <Link2 size={11} /> Replace Image (URL)
      </button>
      {showUrlInput && (
        <div className="mt-1 space-y-1">
          <input type="text" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && loadFromUrl()}
...
function TableProperties({ el, update }: { el: any; update: (u: any) => void }) {
  ...
  return (
    <>
      ...
      <PropRow label="Border"><ColorInput value={el.borderColor || '#C0C0C0'} onChange={(v) => update({ borderColor: v })} /></PropRow>
      <div className="flex gap-1">
        <button onClick={() => {
          const newRows = [...(el.rows || [])]; const cols = el.columns?.length || 4;
          newRows.push({ height: 30, cells: Array.from({ length: cols }, () => ({ content: '' })) });
          update({ rows: newRows, height: el.height + 30 });
        }} className="flex-1 px-2 py-1 text-[10px] bg-editor-bg text-editor-text rounded hover:bg-editor-hover">+ Row</button>
        <button onClick={() => {
          const newCols = [...(el.columns || [])]; newCols.push({ width: 100 });
          const newRows = (el.rows || []).map((r: any) => ({ ...r, cells: [...r.cells, { content: '' }] }));
...
function NumberInput({ value, onChange, step = 1 }: { value: number; onChange: (v: number) => void; step?: number }) {
  return (
    <input type="number" value={Math.round(value * 100) / 100} onChange={(e) => onChange(parseFloat(e.target.value) || 0)} step={step}
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <input type="text" value={value || ''} onChange={(e) => onChange(e.target.value)}
        className="flex-1 min-w-0 bg-editor-bg text-editor-text text-[10px] border border-editor-border rounded px-1.5 py-0.5 font-mono focus:border-editor-accent outline-none" placeholder="#000000" />
    </div>
  );
}
    ...
    <div className="flex gap-0.5">
      {options.map((opt) => (
        <button key={opt.value} onClick={() => onChange(opt.value)}
          className={`flex-1 px-1 py-0.5 text-[10px] rounded transition-colors ${value === opt.value ? 'bg-editor-accent text-white' : 'bg-editor-bg text-editor-text hover:bg-editor-hover'}`}>
          {opt.label}
        </button>
      ))}
    </div>
...
function ToggleButton({ active, onClick, label, bold, italic, underline, strike }: {
  ...
  return (
    <button onClick={onClick}
      className={`w-7 h-7 flex items-center justify-center rounded text-xs transition-colors ${active ? 'bg-editor-accent text-white' : 'bg-editor-bg text-editor-text hover:bg-editor-hover'}`}
      style={{ fontWeight: bold ? 'bold' : undefined, fontStyle: italic ? 'italic' : undefined, textDecoration: underline ? 'underline' : strike ? 'line-through' : undefined }}>
      {label}
    </button>
  );
}
